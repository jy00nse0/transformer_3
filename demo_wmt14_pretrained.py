...
def parse_args():
    ...
    parser.add_argument(
        ...
        help='Path to last checkpoint to resume training from. '
             'The training will continue from the saved step. (default: None)'
    )
...
def main():
    ...
    # 7.5. 체크포인트 로드 (resume training)
    # ------------------------------------------------------------------------

    initial_step = 0
    initial_epoch = 0
    
    if args.last_checkpoint_path:
        print_section("체크포인트 로드 (학습 재개)")
        
        print(f"Loading checkpoint from: {args.last_checkpoint_path}")
        
        if not os.path.exists(args.last_checkpoint_path):
            raise FileNotFoundError(f"Checkpoint file not found: {args.last_checkpoint_path}")
        
        checkpoint = torch.load(args.last_checkpoint_path, map_location=device)
        ...
        # Get the step from checkpoint
        initial_step = checkpoint['step']
        initial_epoch = checkpoint.get('epoch', 0)
        
        print(f"✓ Checkpoint loaded successfully")
        print(f"  Saved step: {initial_step:,}")
        print(f"  Saved epoch: {initial_epoch}")
        
        # Validate that we haven't already reached max_steps
        if initial_step >= args.max_steps:
            print(f"\n⚠️  Warning: Training already completed or exceeded max_steps!")
            print(f"  Checkpoint step ({initial_step:,}) >= max_steps ({args.max_steps:,})")
            print(f"  Nothing to train. Exiting.")
            return
        
        # Calculate remaining steps (for display only)
        remaining_steps = args.max_steps - initial_step
        
        print(f"\nResuming training:")
        print(f"  Target max_steps: {args.max_steps:,}")
        print(f"  Starting from step: {initial_step:,}")
        print(f"  Remaining steps: {remaining_steps:,}")
        print(f"\n✓ Training will resume from step {initial_step:,} to step {args.max_steps:,}")
    ...
    global_step = initial_step  # Start from checkpoint step if resuming
    epoch = initial_epoch  # Start from checkpoint epoch if resuming
